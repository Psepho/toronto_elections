...
toronto_map +
  ...
  facet_wrap(~year)
```
```{r position_change, echo=FALSE, cache=TRUE}
...
In order to analyze the upcoming election, we use previous election results and apply:
...
This does not yet say _why_ voters vote the way they do (which we hope comes next as we add additional complexity), but does identify _where_ those voters may be.
...
```{r issue_setup,echo=FALSE, cache=TRUE}
issues_df <- vote_history  %.%
  group_by(ward, area, candidate, add = FALSE) %.%
  summarize(votes=sum(total_votes), airport_expansion=mean(`Airport expansion`), finance_budget=mean(`Finance & Budget`), transit=mean(Transit), transportation=mean(Transportation), waste_management=mean(`Waste management`))
issues_df$ward <- as.factor(issues_df$ward)
issues_df <- group_by(issues_df[,c(1,4:9)], ward)

areas_per_ward <- geo_2014 %.%
#   filter(year == "2014") %.%
  group_by(ward, area) %.%
  summarize(count = n()) %.%
